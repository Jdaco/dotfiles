...
(after! ivy
  (defun find-place () (interactive)
         (let* ((candidates (org-map-entries
           ...
           (ivy-read "Places: " candidates
                     :action '(1
                               ("s" (lambda (place) (org-ql-search 'org-agenda-files `(children (property "Place" ,place)) :super-groups '((:auto-category t)))) "search")
                               ("c" (lambda (place) (kill-new place)) "copy")))))

  (defun find-areas (&optional action)
    (interactive)
    (let* ((candidates (directory-files (f-join org-directory "areas") t "^[^.].*\.org")))
           (ivy-read "Areas: " candidates
                     :caller 'find-areas
                     :action (or action #'find-file))))

(ivy-configure 'find-areas
  :display-transformer-fn #'file-name-base)
...
(setq org-ql-views '(("Visions"
                    ...
                    ("Work Queue"
                     ...
                     :query (and
                             ...
                             (not (tags "ARCHIVE" "recipe" "certification" "book" "goal" "vision" "training"))
...
(setq org-roam-tag-sources '(prop last-directory)
      ...
      org-roam-capture-templates '(("d" "default"
                                   ...
                                   ("P" "Presentation"
                                    plain #'org-roam--capture-get-point "%?"
                                    :file-name "presentations/%<%Y%m%d%H%M%S>-${slug}"
                                    :head "#+title: ${title}\n\n- tags :: "
                                    :unnarrowed t)
...
; Autoload ivy-bibtex so capture template works on startup
(require 'ivy-bibtex)
#+end_src
...
(defun ivy-bibtex-find-key ()
    (require 'bibtex-completion)
    (bibtex-completion-init)
    (with-temp-buffer
        (ivy-read "Bibtex: " (bibtex-completion-candidates)
                  :caller 'ivy-bibtex
                  :action #'ivy-bibtex-insert-key)
        (buffer-string)))
...
("Book" :keys "b" :function (lambda () (goto-or-create-heading "Books"))
                 :template (lambda () (bibtex-completion-fill-template
                                       (bibtex-completion-get-entry (ivy-bibtex-find-key))
                                       (with-current-buffer
