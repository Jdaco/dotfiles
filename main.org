#+TITLE: Dotfiles
#+VISIBILITY: overview
#+PROPERTY: header-args :mkdirp t :noweb yes
...
(let ((system (get-system-name)))
  (cond ((string= system "alpha")
         '(progn
           <<alpha-configs>>
           ))
        ((string= system "archlinux")
         '(progn
           <<archlinux-configs>>
           ))
        ))
...
#+begin_src elisp :noweb-ref archlinux-configs
...
#+begin_src text :tangle ~/.config/doom/eshell/aliases
addpath ~/.local/bin
...
Symlink your main users ~/.gnupg directory
...
#+begin_src shell :shebang "#!/usr/bin/env bash" :tangle ~/.local/bin/key-init
set -e
...
#+begin_src shell :shebang "#!/usr/bin/env bash" :tangle ~/.local/bin/key-backup
set -e
...
**** Arch Laptop
My Arch laptop isn't suing Guix so the XSession needs to be created manually
...
*** mpv
:PROPERTIES:
:ID:       0e05478d-b9f7-4ca0-a3b4-cd2343788ec4
:END:
#+begin_src text :tangle ~/.config/mpv/mpv.conf
...
I'm only using =org-roam= on my Alpha machine at the moment

#+begin_src elisp :noweb-ref alpha-configs :results none
...
:END:
#+begin_src conf :tangle ~/.config/htop/htoprc
...
#+begin_src text :noweb-ref maildir
~/mail
#+end_src

Once you have ~offlineimap~ and ~mu~ install you'll need to run the below commands to download and index your emails
