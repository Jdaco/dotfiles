...
#+name: system-configs
#+begin_src elisp
(cond ((string= machine "alpha")
        '(progn
        <<alpha-configs>>
        ))
    ((string= machine "archlinux")
        '(progn
        <<archlinux-configs>>
        ))
    )
...
addpath ~/.local/bin ~/dotfiles/bin
...
#+begin_src :tangle ~/.config/autorandr/desktop/setup
...
#+end_src
* Git
#+begin_src conf :tangle (when (string= machine "alpha") "~/.config/git/config")
[user]
    name = Chaise Conn
