...
(map! :nv "M-a" #'evil-scroll-up
      :nv "M-;" #'evil-scroll-down
      )

(after! key-chord
  (key-chord-define-global "dk" 'evil-write-all)
  (key-chord-mode)
  )
                ...
                ("Book" :keys "b" :function (lambda () (goto-or-create-heading "Books"))
                 :template (lambda () (bibtex-completion-fill-template
                                       ...
                                       (with-current-buffer (find-file-noselect "~/roam/areas/org/templates/book.org")
                                         (buffer-string)))))
                ("New Cookie" :keys "C"
                 :function (lambda() (find-file (f-join "~/cookies" (format "%s.org" (read-string "Cookie Name: ")))))
                 :template-file "~/cookies/cookie.org")
                ("Cookies" :keys "c"
                 ...
                 :visit-file "Makefile"
                 :children (
                            ("Mkosi" :keys "m" :template-file "~/cookies/mkosi.org")
                            ("Terraform" :keys "t"
                             :children (("Environment"  :keys "e" :template-file "~/cookies/terraform/environment.org" :git nil)
                                        ("Project"  :keys "p" :template-file "~/cookies/terraform/project.org"
                                         :gitignore ("terraform" "terragrunt"))
                                        ("Remote State" :keys "r" :git nil
                                         :children (("AWS" :keys "w" :template-file "~/cookies/terraform/remote-state/aws.org")
                                                    ("Azure" :keys "a" :template-file "~/cookies/terraform/remote-state/azure.org"))))))))))
  )
