...
(doom! :input
       ...
       (org +noter +pandoc +roam2 +gnuplot +hugo +crypt)               ; organize your plain life in plain text
...
(map! :nvm "M-a" #'evil-scroll-up
      ...
      :n "g s d" #'avy-kill-region
      :n "g M" #'avy-move-region
      )
#+end_src

[[https://github.com/edkolev/evil-lion][evil-lion]] allows you to align fields around delimeters

#+begin_src elisp :noweb-ref packages
(package! evil-lion)
#+end_src

[[https://github.com/mamapanda/evil-owl][evil-owl]] shows your marks and registers when you try to use them
...
I use the doom function =(+format/buffer)= to format my files

You can configure the formatter for a given mode using =(set-formatter!)=

#+begin_src emacs-lisp
(set-formatter! 'unique-name '("command" "line" "here") :modes '(name-of-major-mode))
#+end_src
**** Formatting src blocks

Currently the only way to format src blocks is to...

1. =(org-edit-src-code)= :: Open source in a new buffer
2. =(+format-buffer)= :: Format the buffer
3. =C-C= :: Save the buffer

TODO: automate src block formatting
*** Auto Revert

I use syncthing to sync files between machines, and =global-auto-revert-mode= will automatically reload files from the filesystem when they change

#+begin_src elisp :noweb-ref configs
(global-auto-revert-mode t)

(map! :leader
      "b a" (cmd! (auto-revert-buffers)))
#+end_src
...
(map! :leader
      :prefix "o C-e"
      "l" #'+eshell/split-right
      "j" #'+eshell/split-below
      )
(map! :map eshell-mode-map
      "M-h" #'esh-help-run-help
      :i "C-k" #'eshell-previous-matching-input-from-input
      :i "C-j" #'eshell-next-matching-input-from-input
      :n "C-k" #'eshell-previous-prompt
      :n "C-j" #'eshell-next-prompt
      :ni "C-S-k" #'consult-history
      )
#+end_src
...
(map! :map eshell-mode-map
      ...
      :ni "C-h" (cmd! (eshell/cd "..") (eshell-interactive-print "\n") (eshell-emit-prompt))
      ;; Move back to previous directory
      :ni "C-l" (cmd! (eshell/cd "-") (eshell-interactive-print "\n") (eshell-emit-prompt))
      )
#+end_src

#+begin_src shell :noweb-ref aliases
alias cds cd $(ccc/find-file-in default-directory nil "directory")
alias cdS cd $(ccc/find-file-in "~/" nil "directory")
#+end_src

***** eshell-up

[[https://github.com/peterwvj/eshell-up][eshell-up]] makes it eash to move up to a certain parent directory
...
[[https://github.com/xuchunyang/eshell-z][eshell-z]] lets you jump to a recently used directory
...
These are packages that are added by Doom that I don't want

#+begin_src elisp :noweb-ref packages
(disable-packages! eshell-did-you-mean fish-completion)
#+end_src
...
alias ls eza --long --color=always --icons --group-directories-first $*
alias lsr ls --tree $*
alias lsrd lsr --only-dirs $*
alias lsa ls -a $*
alias empty-trash ccc/empty-trash && sync && df
alias bat bat --paging=never $*
alias m sudo mount $1 {mktemp -d} && cd $_
...
#+begin_src elisp  :noweb-ref configs
(defun ccc/eshell-prompt-git-branch ()
  "Get the eshell prompt text for the current git branch"
  (let ((branch (magit-name-branch "HEAD")))
    (if branch
      (format "[󰊢 %s] " branch)
    "")))

(defun ccc/eshell-prompt-aws-profile ()
  "Get the eshell prompt text for the current AWS profile"
  (let* ((profile (getenv "AWS_PROFILE"))
         (is-default (or (not profile) (string-equal profile "default"))))
    (if (not is-default)
      (format "[󰸏 %s] " profile)
        ""
    )))

(defun ccc/eshell-prompt ()
  (let* ((branch (ccc/eshell-prompt-git-branch))
         (directory (abbreviate-file-name (eshell/pwd)))
         (success (zerop eshell-last-command-status))
         (aws (ccc/eshell-prompt-aws-profile)))
    (format "\n%s %s%s%s "
            (propertize directory 'face '+eshell-prompt-pwd)
            (propertize aws 'face 'magit-branch-current)
            (propertize branch 'face 'magit-branch-current)
            (propertize "λ" 'face (if success 'success 'error))
            )))

(after! eshell
    (require 'magit)
    (setq eshell-prompt-function #'ccc/eshell-prompt))
#+end_src
**** Snippets
:PROPERTIES:
...
(defun ccc/find-bookmark-handler (record)
  (let ((search-dir (bookmark-prop-get record 'location))
    ...
    (fd-dired search-dir search-pattern)))
...
*Note:* Disabling treemacs in favor of =dired-sidebar= until I have a reason not to

**** Projects

TODO: Look into using [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Projects.html][project.el]]
...
(defun ccc/dired-rename (&optional arg)
  "When ARG is set, set dired-dwim-target to nil so the prompt stays in the current dired location"
  (interactive "P")
  (let* ((dired-dwim-target (if arg nil dired-dwim-target)))
    (dired-do-rename)
    ))
...
(map! :map dired-mode-map
      ...
      "C-S-d" (cmd! () (let ((delete-by-moving-to-trash nil)) (dired-do-delete)))
      "Z" #'dired-do-compress
      "c" #'dired-do-compress-to
      "C-S-d" (cmd! () (let ((delete-by-moving-to-trash nil)) (dired-do-delete)))
      "C-S-g" (cmd! () (call-interactively #'dirvish-emerge-mode))
      :localleader
...
(defun dired-scale-text ()
  (text-scale-set 2))

(after! dired-x
    (add-hook! 'dired-mode-hook #'dired-scale-text)

    ;; Overwrite dired-next-line because for some reasno when
    ;;   dired-hide-details-mode was active it was moving down 2 lines
    ...
    (setq dired-listing-switches "-alh --group-directories-first"
        dired-omit-files "^\\."
        dired-hide-details t
        dirvish-hide-details t
        dirvish-attributes '(vc-state nerd-icons subtree-state)
        dirvish-emerge-groups '(("Directories" (predicate . directories))
                                ("Org" (extensions "org"))
                                ("Data" (extensions "csv" "json" "tsv" "parquet" "orc"))
                                ("Documents" (extensions "pdf" "docx" "xlsx"))
                                ("Code" (extensions "scm" "el" "py" "go" "orc"))
                                ("Images" (extensions "jpg" "jpeg" "png" "gif" "svg"))
                                ("Videos" (extensions "mp4" "mkv" "webm" "flv" "m4v")))
        ))
...
(defun ccc/dired-mark-dangling-url ()
  "Mark in Dired the .url files that don't have another file by the same name

This is usefule for removing .url files for videos that I've already watched and deleted "
  (interactive)
  (dired-map-dired-file-lines
   (lambda (file)
     (when (and
            (s-ends-with? ".url" file)
            (= 1 (length (f-glob (s-concat (s-replace "[" "[[]" (f-base file)) ".*")))))
...
#+begin_src elisp :noweb-ref packages
; I need dirvish-open-with-programs until Emacs 30 is available
; https://github.com/alexluigit/dirvish/commit/362601154d3aa13ed60d78be2e1bfa2c166a5e37
(package! dirvish :pin "77299abeeb5f518671d71447d1d061edeaf9537b")
#+end_src
...
I use [[https://github.com/DerBeutlin/filetags.el][filetags.el]] as an interface to tag dired files. The tags are stored in the file name.

Search file tags is as simple as filtering with ~find-file~

#+begin_src elisp :noweb-ref configs
(map! :map dired-mode-map
      :n "T" #'filetags-dired-update-tags)
#+end_src
...
#+begin_src elisp :noweb-ref configs
(map! :map embark-file-map
      :desc "Find in directory" "C-f" (lambda (file) (find-file (ccc/find-file-in (if (f-directory-p file) file (f-dirname file)))))
      :desc "Open in window" "W" (cmd! (aw-switch-to-window (aw-select nil)) (call-interactively #'find-file))
      )
#+end_src
  ...
  (when (posframe-workable-p)
    (let* ((posframe
	    (posframe-show buffer
                           ; A bug in newer emacs versions causes the frame height to miscalculated
                           ; Set height manually as a workaround
                           ; https://github.com/yanghaoxie/transient-posframe/issues/5
...
Doom eamcs uses =straight.el= to manage packages

*** Keys

#+begin_src elisp :noweb-ref configs
(map! :leader
      "h p" #'doom/help-packages)

(map! :map embark-package-map
      ; Open the homepage for the package
